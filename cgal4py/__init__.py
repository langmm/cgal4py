...
def run_nose(verbose=False):
    ...
    if os.path.samefile(os.path.dirname(my_package_dir), initial_dir):
        ...
        raise RuntimeError(
            """
    The cgal4py.run_nose function does not work correctly when invoked in
    the same directory as the installed cgal4py package. Try starting
    a python session in a different directory before invoking cgal4py.run_nose
