...
def test_locate():
    ...
    for c in T.finite_cells:
        ...
        # assert(c.facet(0).is_equivalent(T.locate(c.facet(0).edge(0).center, c)))
...
def test_edge():
    ...
    for e in T.all_edges:
        ...
        if e.is_unique:
            count_wrap += 1
            print(e.is_unique, e.has_offset, off1, off2, e.vertex1.periodic_offset, e.vertex2.periodic_offset)
        if not e.has_offset:
