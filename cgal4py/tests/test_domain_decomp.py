...
def test_process_leaves():
    ...
    tree3 = domain_decomp.tree('kdtree', pts3, left_edge3, right_edge3, 
                               periodic=False, leafsize=leafsize)
    leaves2 = domain_decomp.process_leaves(tree2.leaves, left_edge2, right_edge2, False)
    leaves3 = domain_decomp.process_leaves(tree3.leaves, left_edge3, right_edge3, False)
    # Without information
    ...
    right_edges = np.array([[0.5,0.5],
                            ...
                            [1.0,0.5],
                            [1.0,1.0]], 'float')
    leaves = [domain_decomp.GenericLeaf(5, left_edges[i,:], right_edges[i,:]) for i in range(2)]
    leaves = domain_decomp.process_leaves(leaves, left_edge2, right_edge2, False)
...
def test_GenericTree():
    ...
    right_edges = np.array([[0.5,0.5],
                            ...
                            [1.0,1.0]], 'float')
    leaves = [domain_decomp.GenericLeaf(5, left_edges[i,:], right_edges[i,:]) for i in range(2)]
    tree = domain_decomp.GenericTree(np.arange(5*left_edges.shape[0]).astype('int'),
